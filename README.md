...
- **SEO:** sitemap auto-generated by `@astrojs/sitemap`; `robots.txt` links to it.
  Update the domain in `robots.txt` if you move off the Vercel URL. Pages also
  emit JSON-LD (`ProfessionalService` site-wide + `FAQPage` on Services).
- **Dark mode:** header toggle (sun/moon) switches between frost (light) and the
  night-sky aurora (dark); the choice persists. Dark tokens live in `global.css`
  under `html[data-theme="dark"]`.
- **Services extras:** the "How it works" steps and FAQ are plain arrays at the top
  of `src/pages/services.astro` — edit the text freely.
- **Testimonials:** add quotes to `TESTIMONIALS` in `src/config.ts`; the section on
  the home page stays hidden until there's at least one.
